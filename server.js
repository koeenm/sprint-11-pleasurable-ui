...
// Declare the variable to hold advertisement data
let allAdvertisementsData = [];

// Data ophalen van de API
fetchData().then((data) => {
  allAdvertisementsData = data;

  // Add slugs to each service
  ...
  app.post('/search', function (request, response) {
    ...
    // Pagination dinges logic woop
    const page = parseInt(request.query.page) || 1;
    const itemsPerPage = 6;
    const startIndex = (page - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const totalPages = Math.ceil(filteredServices.length / itemsPerPage);
    const servicesOnPage = filteredServices.slice(startIndex, endIndex);
    const noResultsMessage = filteredServices.length === 0 ? 'Geen resultaten gevonden.' : null;
  ...
  app.get('/overzicht', function (request, response) {
    ...
    if (searchQuery) {
      ...
      noResultsMessage = servicesOnPage.length === 0 ? 'Geen resultaten gevonden.' : null;
    } else {
  ...
  app.post('/like', function (request, response) {
    ...
    const service = allAdvertisementsData.find((service) => service.id === parseInt(likeId));
    if (!service) {
